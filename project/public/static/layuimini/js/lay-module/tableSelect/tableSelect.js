layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    tableSelect.prototype.render = function (opt) {
        ...
        if(opt.field){
            ...
            //创建一个隐藏域用于保存选中的数据
            var selectedInput = $(`<input type="hidden" name="${opt.field}" class="layui-input">`);
            elem.after(selectedInput);
            selectedInput.val(elem.attr('ts-selected'));         
        }
        /****Brightness */
        ...
        elem.off('click').on('click', function(e) {
            ...
            /***********Brightness************* */
            opt.table. parseData = function (res) { //res 即为原始返回的数据
                return {
            ...
            function selectDone (checkStatus){
                if(opt.checkedKey){
                    ...
                    /*******Brightness******** */
                    selectedInput&&selectedInput.val(selected);
                    /********Brightness*** */
     ...
     * 获取选中值
     */
      tableSelect.prototype.getSelected = function () {
          let selected = this.elem.attr('ts-selected');
